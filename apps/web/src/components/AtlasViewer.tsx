...
export default function AtlasViewer({ sheetUrl, atlas, poseFilter }: Props) {
  ...
  // Compute overlay rects and clamp to sheet bounds
  const rects = useMemo(() => {
    const W = atlas?.meta?.size?.w ?? Number.MAX_SAFE_INTEGER
    const H = atlas?.meta?.size?.h ?? Number.MAX_SAFE_INTEGER
    return frames
      .map((f) => ({
        key: `${f.pose}-${f.index}`,
      ...
      .filter(r => r.x >= 0 && r.y >= 0 && r.w > 0 && r.h > 0 && r.x + r.w <= W && r.y + r.h <= H)
  }, [frames, atlas])
  ...
  {atlas?.meta?.fixed_cell ? <span className="text-emerald-400">fixed</span> : <span className="text-zinc-400">auto</span>}
      ...
      <div
        ...
        style={{ maxHeight: 640 }}  // more breathing room
      >
        <div
          className="relative"
